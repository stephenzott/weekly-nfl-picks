...
- **External data (optional, both should have manual fallback in the admin UI):**
  ...
  - **ESPN's public scoreboard endpoint** for final scores, to help settle bets automatically: `https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard`. No API key required. This is an unofficial/undocumented endpoint — stable for years but could change without notice, so always keep manual score entry as a fallback in the admin tab. **Implementation note, added 2026-09-11:** in addition to the admin's manual "Fetch Score" button, a scheduled **GitHub Actions** workflow (`.github/workflows/update-scores.yml`, running `scripts/update-scores.mjs` every 30 minutes) now polls this same endpoint for any game past kickoff still marked `scheduled`, writes the final score, and re-runs settlement against every pick — so results go final even if nobody opens the app afterward. This is genuinely scheduled server-side automation, not a client-side "check on load" trick like the missed-pick backfill in 4.3; it does NOT cover Super Bowl Props or Season Win Totals settlement, since those settle off a value an admin types in by hand (no new data for a scheduled job to react to).
...
A regular season week is capped at 6 total *required picks* per user: AM, PM, SNF, MNF, Wild Card, and Highest Spread. `WildCardPool` games themselves are NOT capped — they're just candidates feeding the one shared Wild Card pick and one shared Highest Spread pick, so the pool can hold as many leftover games as the admin wants to import without changing how many picks a user makes that week. The 6-game cap enforced in the Admin UI (`AddGameForm`) applies only to AM/PM/SNF/MNF/Bonus games — each of those creates its own separate required pick, so those are what the cap is protecting against. [Implementation note: added 2026-09-08 per Stephen; revised 2026-09-24 per Stephen to exempt `WildCardPool` games from the cap after the original wording caused the pool to be starved down to 1-2 games in practice. The 2026-09-08 version of this note said a week with AM/PM/SNF/MNF plus 2 Bonus games would have no room for any `WildCardPool` games at all — that is no longer true.]
...
- **Missed pick = coin-flip auto-pick.** If a user hasn't submitted a pick for a game before it kicks off, the system records a pick for them automatically: a coin flip decides which side of the spread (and, for a WildCard-style slot with multiple candidate games, which game), $10 stake, settled normally — it can win, lose, or push just like a real pick — clearly flagged as auto-generated (not a pick the user actually made) so it displays differently in the UI (e.g., grayed out, labeled). It still counts toward the win/loss/push "record" in standings, same as any other pick. If this is the LAST required slot the user has left for the week (every other slot already has a real or auto-picked value), its stake is forced to whatever's left of the week's budget instead of $10, so a user who misses their whole week still lands on exactly $120 total — same rule as a user's own last real pick. Since this is a static site with no always-on server, this check does not need to run instantly at kickoff — it's acceptable for it to run client-side whenever any user next opens the app (i.e., on load, scan for games whose kickoff has passed with no submitted pick from any user, and backfill auto-picks for those). **Implementation note, changed 2026-09-11 (Stephen):** originally a flat $10 automatic loss (no coin flip, never a real bet) — see `src/lib/missedPicks.ts` for the full reasoning behind the change, and its rare fallback (a game that never got a spread set at all still records as the old flat $10 loss, since there's no side to flip a coin for). Not applied retroactively to picks already auto-generated under the old rule.

**Implementation note, added 2026-09-11:** a coin flip (or a flat-loss fallback) is a placeholder, not necessarily what the user actually meant to pick. The Admin tab's "Add a Missing Pick" control (`AddMissingPickSection.tsx`) lets an admin fill in — or override — the real pick for any slot a user never got to before kickoff, whether or not an auto-pick already exists there. It writes to the exact same deterministic document ID (`userId_weekId_pickType`, see `src/lib/picks.ts`) the auto-pick would use, so it works identically whether the slot is empty or already holds a coin-flipped placeholder — no need to tell those two cases apart first.
...
**Confirmed intentional (2026-09-08):** unlike weekly picks (which lock per-game at kickoff), there is NO deadline/lock on Season Win Totals at all — an admin can add or edit a bet at any time, including after the season has started. Stephen confirmed this should stay honor-system, matching the app's overall no-auth trust model (Section 3), rather than adding an enforced cutoff. **Implementation note, added 2026-09-11:** "edit" above originally only meant entering `actualWins` once the season's real result was known. The Admin tab's Season Win Totals section now also has a separate "correct this bet" control (`SeasonWinTotalsSection.tsx`) to fix the bet's own `team`/`line`/`side`/`stake` if it was typed in wrong — a distinct action from recording the real outcome, so it gets its own edit state in the UI rather than sharing one "editing" toggle.
...
- Enter/confirm final scores to settle bets (auto-fill attempt via ESPN scoreboard endpoint, always manually editable/overridable) — plus a fully automated path that needs no admin action at all, see Section 2's ESPN bullet (added 2026-09-11)
...
- **Correct a locked pick or a saved Season Win Total bet** (added 2026-09-11): picks lock client-side after kickoff (`PickSlot.tsx`) with no other in-app way to fix a wrong side or stake afterward — this is UI-only, not enforced by `firestore.rules` (see Section 3), but until this control existed there was still no path in the app itself to correct one. `EditPicksSection.tsx` covers game picks (including overriding a coin-flip auto-pick, other than its rare flat-loss fallback with no real side to correct); the Season Win Totals correction is the `SeasonWinTotalsSection.tsx` control described in 4.4's implementation note.
- **Fill in a pick that was never saved before kickoff** (added 2026-09-11, `AddMissingPickSection.tsx`): see 4.3's implementation note — recovers the case where a user forgot to submit entirely, using the same deterministic pick document ID as the normal picks screen and the missed-pick auto-pick.
...
- `isAutoPick`: boolean (true if this was auto-generated from a missed pick — **renamed from `isDefaultLoss` 2026-09-11** when the missed-pick rule changed from a flat automatic loss to a settled coin-flip bet, since the field no longer implies a loss)
